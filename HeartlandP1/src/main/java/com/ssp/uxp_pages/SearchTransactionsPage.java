...
public class SearchTransactionsPage extends LoadableComponent<HomePage> {
	 ...
	 *            : Webdriver
	 */
	public SearchTransactionsPage(WebDriver driver, ExtentTest report) {
		this.driver = driver;
	...
	protected void isLoaded() {
		...
		if (!driver.getTitle().contains("SSP - Pure Insurance")) {
			Log.fail("Search transactions Page did not open up. Site might be down.", driver, extentedReport);
		} else {
			Log.message("Successfully navigated to Search Transactions Page", driver, extentedReport);
		}
	}
